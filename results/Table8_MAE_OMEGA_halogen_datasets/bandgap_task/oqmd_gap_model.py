...
from modnet_gnn.featurizers.presets.modnet_omega_2023 import MODNetOmegaFast2023, MODNetOmega2023
from modnet_gnn.matbench.benchmark import matbench_kfold_splits
from modnet_gnn.hyper_opt.fit_genetic import FitGenetic
...
def megnetMM_featurizer(path_full_featurized_data):
    from modnet_gnn.featurizers.presets.modnet_omega_2023 import MEGNetFeaturizer
    md = MODData.load(path_full_featurized_data)
    MEGNetAdjacent = MEGNetFeaturizer(model_type='MatMinerEncoded_v1')
    df_mmfeat= MEGNetAdjacent.get_features(md.df_structure['structure'])
    # save df_adjfeat
    df_mmfeat.to_pickle('df_mmfeat.pkl')
...
def MMchange_featurized_data(path_full_featurized_data, path_dfMM):
    # load data
    ...
    dfMM = pd.read_pickle(path_dfMM)
    # concatenate
    df = pd.concat([dfMM,df],axis=1)
    print(df)
    path_tosave = "full_featurized_dataset/oqmdhalides_widebandgap_featurizedOmegaFastROSA_complete"
    # drop structure
    df.drop(columns=['structure'], inplace=True)
    # save
    data.df_featurized = df
    data.save(path_tosave)

def process_featurized_data(path_full_featurized_data, job_prefix='', to_drop='', n_split=20, classification=False):
    ...
    featurized_data_fname = f'full_featurized_dataset/{job_prefix}_featurized'
    ...
    for ind, (train, test) in enumerate(matbench_kfold_splits(data,n_splits=n_split, classification=classification)):
...
def main(job_prefix, target_name, n_folds=5, classification=False, n_jobs=24):
    ...
    if not os.path.isfile(featurized_data_fname):
        ...
        md = MODData(materials = full_data['structure'],
                        targets = full_data[target_name].values,
    ...
    # sample using from_indices, this is just for testing
    # indices = np.random.choice(len(md.df_featurized), 200, replace=False)
    # md = md.from_indices(indices)
    # ##############################
    ...
    #### pick only 50 columns randomly to make it faster
    ...
    ga_settings = {"size_pop": 10, "num_generations": 5, "n_jobs": n_jobs, 
                    "refit": 5, "nested": 5 }
    ####### to make it faster, we can use the following settings
    # ga_settings = {"size_pop": 2, "num_generations": 2, "n_jobs": n_jobs, 
    #             "refit": 2, "nested": 2 }
    #######
    # split data into folds
    fold_data = []
    for ind, (train, test) in enumerate(matbench_kfold_splits(md, n_splits=n_folds, classification=classification)):
    ...
    for fold_number, fold in enumerate(fold_data):
        ...
        model_name = f'full_featurized_dataset/MODNet_{job_prefix}_bestGA_f{fold_number}'
    ...
    if not os.path.isfile(f'final_model/MODNet_{job_prefix}_final_ensemble'):
        for fold_number in range(5):
            model = EnsembleMODNetModel.load(f'full_featurized_dataset/MODNet_{job_prefix}_bestGA_f{fold_number}')
            models.append(model)
    ...
    if not classification:
        ...
        for i in range(num_sets):
            ...
            predicted_stability1 = result1[target_name]
            actual_stability = md_train.df_targets[target_name]
            ...
            avg_stds = np.mean(stds1[target_name])
        ...
        test_predicted = test_results[target_name]
        test_actual = ensemble_test_data.df_targets[target_name]
        ...
        avg_test_std = np.mean(test_stds[target_name])
        # print the metrics
        print("\nTest Set Metrics:")
...
if __name__ == "__main__":
    ...
    # path_full_featurized_data = "full_featurized_dataset/oqmdhalides_widebandgap_featurizedOmegaROSA_complete"
    # job_prefix = "oqmdhalides_widebg_omegaROSAG"
    # to_drop=''
    # process_featurized_data(path_full_featurized_data, job_prefix=job_prefix, to_drop=to_drop, n_split=50)
    # main(job_prefix, target_name='band_gap', n_folds=10)

    # path_full_featurized_data = "full_featurized_dataset/oqmdhalides_widebandgap_featurizedOmegaROSA_complete"
    # job_prefix = "oqmdhalides_widebg_matminer"
    # to_drop = 'pretrained_megnet+ROSA+OFMencoded+adjacent+G+bondfractions' 
    # process_featurized_data(path_full_featurized_data, job_prefix=job_prefix, to_drop=to_drop, n_split=50)
    # main(job_prefix, target_name='band_gap', n_folds=10)    

    # path_full_featurized_data = "full_featurized_dataset/oqmdhalides_widebandgap_featurizedOmegaROSA_complete"
    # job_prefix = "oqmdhalides_widebg_matminerBondFrac"
    # to_drop = 'pretrained_megnet+ROSA+OFMencoded+adjacent+G' 
    # process_featurized_data(path_full_featurized_data, job_prefix=job_prefix, to_drop=to_drop, n_split=50)
    # main(job_prefix, target_name='band_gap', n_folds=10)   

    # path_full_featurized_data = "full_featurized_dataset/oqmdhalides_widebandgap_featurizedOmegaROSA_complete"
    # job_prefix = "oqmdhalides_widebg_matminerOFM"
    # to_drop = 'pretrained_megnet+ROSA+adjacent+G+bondfractions' 
    # process_featurized_data(path_full_featurized_data, job_prefix=job_prefix, to_drop=to_drop, n_split=50)
    # main(job_prefix, target_name='band_gap', n_folds=10)

    # path_full_featurized_data = "full_featurized_dataset/oqmdhalides_widebandgap_featurizedOmegaROSA_complete"
    # job_prefix = "oqmdhalides_widebg_matminerOMEG"
    ...
    path_full_featurized_data = "full_featurized_dataset/oqmdhalides_widebandgap_featurizedOmegaEformROSA_complete"
    # MMchange_featurized_data(path_full_featurized_data, "./df_mmfeat.pkl")
    # megnetMM_featurizer(path_full_featurized_data)
    job_prefix = "oqmdhalides_widebg_omega_eform"
    to_drop = 'ROSA+G' 
    process_featurized_data(path_full_featurized_data, job_prefix=job_prefix, to_drop=to_drop, n_split=50)
    main(job_prefix, target_name='band_gap', n_folds=10)

    # path_full_featurized_data = "full_featurized_dataset/oqmdhalides_widebandgap_featurizedOmegaFastROSA_complete"
    # job_prefix = "oqmdhalides_widebg_omegafast"
    # to_drop = 'ROSA+G' 
    # # process_featurized_data(path_full_featurized_data, job_prefix=job_prefix, to_drop=to_drop, n_split=50)
    # main(job_prefix, target_name='band_gap', n_folds=10)
    ...
    # path_full_featurized_data = "full_featurized_dataset/oqmdhalides_widebandgap_featurizedOmegaROSA_complete"
    # job_prefix = "oqmdhalides_widebg_matminerOFMBondFrac"
    # to_drop = 'pretrained_megnet+ROSA+adjacent+G' 
    # process_featurized_data(path_full_featurized_data, job_prefix=job_prefix, to_drop=to_drop, n_split=50)
    # main(job_prefix, target_name='band_gap', n_folds=10)

    # path_full_featurized_data = "full_featurized_dataset/oqmdhalides_widebandgap_featurizedOmegaROSA_complete"
    # job_prefix = "oqmdhalides_widebg_matminerOMEGBondFrac"
    # to_drop = 'ROSA+adjacent+G' 
    # process_featurized_data(path_full_featurized_data, job_prefix=job_prefix, to_drop=to_drop, n_split=50)
    # main(job_prefix, target_name='band_gap', n_folds=10)

    # path_full_featurized_data = "full_featurized_dataset/oqmdhalides_widebandgap_featurizedOmegaROSA_complete"
    # job_prefix = "oqmdhalides_widebg_omega"
    # to_drop = 'ROSA+G' 
    # process_featurized_data(path_full_featurized_data, job_prefix=job_prefix, to_drop=to_drop, n_split=50)
    # main(job_prefix, target_name='band_gap', n_folds=10)

    ## path_full_featurized_data = "full_featurized_dataset/oqmdhalides_bandgap_featurizedOmegaROSA_complete"
    # path_full_featurized_data = "full_featurized_dataset/oqmdhalides_gap_class_featurizedOmegaROSA_complete"
    ...
    # job_prefix = "oqmdhalides_bandgap_matminerROSAG"
    ...
    # job_prefix = "oqmdhalides_gap_class_omegaROSAG"
    # to_drop=''
    # process_featurized_data(path_full_featurized_data, job_prefix=job_prefix, to_drop=to_drop,classification=True, n_split=50)
    # main(job_prefix, target_name='gap_class',classification=True)

    ## job_prefix = "oqmdhalides_bandgap_omega"
    ## to_drop = 'ROSA+G' 
    ## process_featurized_data(path_full_featurized_data, job_prefix=job_prefix, to_drop=to_drop)
    ## main(job_prefix)

    ## job_prefix = "oqmdhalides_bandgap_matminer"
    ## to_drop = 'pretrained_megnet+ROSA+OFMencoded+adjacent+G+bondfractions' 
    ## process_featurized_data(path_full_featurized_data, job_prefix=job_prefix, to_drop=to_drop)
    ## main(job_prefix)
