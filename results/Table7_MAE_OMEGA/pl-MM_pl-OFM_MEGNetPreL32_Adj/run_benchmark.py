...
try:
    from modnet_gnn.featurizers.presets import CompositionOnlyFeaturizer  # noqa
    from modnet_gnn.preprocessing import MODData, CompositionContainer  # noqa
except ImportError:
    pass
...
def featurize(task, n_jobs=1):
    ...
    from modnet_gnn.featurizers.presets import DeBreuck2020Featurizer
...
def benchmark(data, settings, n_jobs=16, fast=False):
    from modnet_gnn.matbench.benchmark import matbench_benchmark
    ...
    from modnet_gnn.models import EnsembleMODNetModel
    return matbench_benchmark(
        ...
        adjacent_model_paths=[ f'./adjacent_models/fold_{i}/MEGNetModel__adjacent.h5' for i in range(5)],	
        n_jobs=n_jobs,
    )
