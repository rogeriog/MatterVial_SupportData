...
def xgb_robust_preselection(data, feature_sets_str, n_jobs=24, target_threshold=800,  
    ...
    while cur_features.shape[1] > target_threshold:
        ...
        X_numeric = X_numeric_full[rename_cols(cur_features.copy()).columns]
...
def run_modnet_training(data_path,
    ...
    df_all.dropna(subset=[internal_target_name], inplace=True)
    final_rows = len(df_all)
    if final_rows < initial_rows:
        print(f"Dropped {initial_rows - final_rows} rows with NaN values in the target column ('{target_name}').")
    ...
    for ind, (train_idx, test_idx) in enumerate(matbench_kfold_splits(md, n_splits=n_folds, classification=is_classification)):
        ...
        if process_this_fold:
            ...
            train_data = xgb_robust_preselection(train_data, f"{target_name}", n_jobs=24, target_threshold=800,   
                                    n_folds=5, consensus_threshold=0.6,   
                                    importance_percentile=20) # Added parameters for robust selection
        ...
        if best_model is None:  # Train if not loaded successfully
            if hyperopt_strategy == "ga":
                ...
                if preset_settings is None:
                    ...
                    if is_classification:
                        # CORRECTED
                        num_unique = train_fold.df_targets[internal_target_name].nunique()
                        model_init_params["num_classes"] = {internal_target_name: num_unique}
                        print(f"  Setting num_classes={internal_target_name} : {num_unique} for classification.")
        ...
        if best_model:
            try:
                ...
                if is_classification:
                    ...
                    # CORRECTED
                    train_true = train_fold.df_targets[internal_target_name].astype(int)
                    test_true = test_fold.df_targets[internal_target_name].astype(int)

                    # Compute Accuracy and F1 Score.
                    # CORRECTED
                    train_accuracy = accuracy_score(train_true, train_pred_class[internal_target_name])
                    train_f1 = f1_score(train_true, train_pred_class[internal_target_name], average='weighted')
                    test_accuracy = accuracy_score(test_true, test_pred_class[internal_target_name])
                    test_f1 = f1_score(test_true, test_pred_class[internal_target_name], average='weighted')

                    # Compute ROC AUC score for binary classification.
                    # CORRECTED
                    if '{}_prob_0'.format(internal_target_name) in train_pred_prob:
                        train_rocauc = roc_auc_score(train_true, 1-train_pred_prob[f"{internal_target_name}_prob_0"])
                        test_rocauc  = roc_auc_score(test_true, 1-test_pred_prob[f"{internal_target_name}_prob_0"])
                    else:
                    ...
                    fold_preds_df = pd.DataFrame({
                        ...
                        'target': test_true,
                        f'pred_class_f{fold_number}': test_pred_class[internal_target_name],
                        f'prob_class1_f{fold_number}': test_pred_prob.get(f"{internal_target_name}_prob_0", np.nan)
                    }).set_index('material_id')
                    all_fold_predictions.append(fold_preds_df)
                    ...
                    # CORRECTED
                    train_true = train_fold.df_targets[internal_target_name]
                    test_true = test_fold.df_targets[internal_target_name]
                    
                    # Compute regression metrics.
                    # CORRECTED
                    train_mae = mean_absolute_error(train_true, train_pred_df[internal_target_name])
                    train_rmse = mean_squared_error(train_true, train_pred_df[internal_target_name], squared=False)
                    train_r2 = r2_score(train_true, train_pred_df[internal_target_name])
                    test_mae = mean_absolute_error(test_true, test_pred_df[internal_target_name])
                    test_rmse = mean_squared_error(test_true, test_pred_df[internal_target_name], squared=False)
                    test_r2 = r2_score(test_true, test_pred_df[internal_target_name])
                    ...
                    fold_preds_df = pd.DataFrame({
                        'material_id': test_fold.structure_ids,
                        'target': test_true,
                        f'prediction_f{fold_number}': test_pred_df[internal_target_name],
                        f'uncertainty_f{fold_number}': test_unc_df[internal_target_name]
                    }).set_index('material_id')
                    all_fold_predictions.append(fold_preds_df)
